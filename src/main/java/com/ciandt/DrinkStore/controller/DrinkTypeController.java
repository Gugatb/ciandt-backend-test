...
import com.ciandt.DrinkStore.service.DrinkTypeService;
...
public class DrinkTypeController {
	 ...
	 */
	private ResponseEntity execute(String pParam1, double pParam2, int pType) {
		Response response = new Response();
		
		try {
			if (pType == 0) {
				response.setResponse(service.add(pParam1, pParam2));
			}
			else if (pType == 1) {
	...
	public ResponseEntity add(
		...
		return execute(name, capacity, 0);
	}
